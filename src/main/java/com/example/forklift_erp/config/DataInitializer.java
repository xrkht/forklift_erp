...
public class DataInitializer implements CommandLineRunner {
    ...
    @Value("${forklift.admin.bootstrap.username:admin}")
    private String bootstrapAdminUsername;

    @Value("${forklift.admin.bootstrap.password:admin123}")
    ...
    private void ensureDefaultSuperAdmin(Role superAdminRole) {
        String username = normalizeBootstrapValue(bootstrapAdminUsername, "forklift.admin.bootstrap.username");
        String password = normalizeBootstrapValue(bootstrapAdminPassword, "forklift.admin.bootstrap.password");
    ...
    private String normalizeBootstrapValue(String value, String propertyName) {
        if (value == null || value.isBlank() || value.contains("${")) {
            throw new IllegalStateException(propertyName + " must be configured before bootstrapping the default admin user");
        }
        return value.trim();
    }
}
